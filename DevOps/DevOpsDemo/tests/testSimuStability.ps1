...
try {
    ...
    Test-Step 'Read Main em.StateCurrent and check if stopped' {
        Assert-PlcConnected
        $mainState = Read-ModuleState -Task 'Main'
        if ($mainState -ne $stoppedState) {
            throw "Main em.StateCurrent is $mainState; expected stopped state $stoppedState."
        }
    }
